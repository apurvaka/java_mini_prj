...
////////////////////Sports table CRUD operations
	...
	public List<AllPlansInfo> getAllPlans() throws Exception {
		...
		Connection con = DbUtil.getConnection();		
		PreparedStatement ps = con.prepareStatement(sql);
		List<AllPlansInfo> list=new ArrayList<>();
		ResultSet rs= ps.executeQuery();
		while(rs.next()){
			
			list.add(new AllPlansInfo(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getDouble(4), rs.getInt(5), rs.getString(6)));
			//System.out.println(rs.getInt("batchId")+" "+rs.getDate("startDate")+" "+rs.getDate("endDate")+" "+rs.getInt("batchSize")+" "+rs.getString("sportName"));
	...
	public int addPlan(Plans plan) throws Exception {
		...
		String sql = "insert into Plans(planId,sportId,planName,fees,duration) values(?,?,?,?,?)";
		Connection con = DbUtil.getConnection();		
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, plan.getPlanId());
		ps.setInt(2, plan.getSportId());
